...
namespace HookControl
{
    public class HookCatchPointBehavior : IHookBehavior
    {
        ...
        private void Back()
        {
            hc.transform.position = Vector2.Lerp(startPos, endPos, current);           
            current += Time.deltaTime / (hc.timePullUpHook * normalazedPercentOfMaxDistance);
            if (current < 1)
            {
                hc.hook.position = endPos;
            }
            if (Vector2.Distance(hc.capturedTarget.transform.position, hc.transform.position) < 0.1f)
            {
                hc.transform.position = endPos;
                hc.hook.transform.position = hc.direction.normalized * hc.idleDistanseHook + (Vector2)hc.transform.position;
